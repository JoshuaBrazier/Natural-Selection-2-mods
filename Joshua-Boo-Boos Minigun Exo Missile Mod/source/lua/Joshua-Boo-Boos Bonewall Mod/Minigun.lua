...
Script.Load("lua/Joshua-Boo-Boos Bonewall Mod/Missile.lua")
...
local function Shoot(self, leftSide)
    ...
    if self.minigunAttacking and not self.overheated and player then
        ...
        if Server then
            local missile = CreateEntity(Sentry_Missile.kMapName, barrel_point, kTeam1Index)
            missile.owner_entity_id = player:GetId()
...
function Minigun:GetBarrelPoint()
    ...
    if player then
        ...
        if player.GetIsLocalPlayer and player:GetIsLocalPlayer() then
...
function Minigun:OnTag(tagName)
    ...
    if self:GetIsLeftSlot() and tagName == "l_shoot" then
        ...
        if self.l_i % fire_rate_multiplier == 0 then
            Shoot(self, true)
        end
        self.l_i = self.l_i + 1
    ...
    elseif not self:GetIsLeftSlot() and tagName == "r_shoot" then
        ...
        if self.r_i % fire_rate_multiplier == 0 then
            Shoot(self, false)
        end
        self.r_i = self.r_i + 1
