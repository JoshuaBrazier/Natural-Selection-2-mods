...
local networkVars =
{
    ...
    carrying_lerk_id = "entityid",

    is_carrying_an_entity = "boolean",

    carried_entity_id = "entityid",

    last_unstuck_time = "time",
...
function Alien:Unstick_Myself(input)

    local teammate_entities = GetEntitiesForTeam("Player", kTeam2Index)
    local alive_teammates = {}
    for i = 1, #teammate_entities do
        if teammate_entities[i]:GetIsAlive() then
            table.insert(alive_teammates[i])
        end
    end

    if #alive_teammates > 0 then
    
        local entity_extents = self:GetExtents()
        if Shared.CollideBox(entity_extents, self:GetOrigin() + Vector(0, 0.05, 0), CollisionRep.Move, PhysicsMask.All, EntityFilterOne(self)) then

            input.move:Scale(0)

            local current_location = self:GetOrigin()
            ...
            repeat
                ...
                local unsure_teammates = GetEntitiesForTeam("Player", kTeam2Index)
                table.removevalue(unsure_teammates, self)
                -- Shared.Message(string.format("# unsure teammates = %i", #unsure_teammates))
                local teammates = {}
                for i = 1, #unsure_teammates do
                    if unsure_teammates[i]:GetIsAlive() then
                        table.insert(teammates, unsure_teammates[i])
                    end
                end
                -- Shared.Message(string.format("teammates contains %i entities", #teammates))
                local distance_and_entity_pairs = {}
                for i = 1, #teammates do
                    local teammate_position = teammates[i]:GetOrigin()
                    local distance = (teammate_position - self:GetOrigin()):GetLength()
                    table.insert(distance_and_entity_pairs, {distance, teammates[i]})
                end

                -- Shared.Message("distances are:")

                -- for i = 1, #distance_and_entity_pairs do

                --     Shared.Message(string.format("distances[%i] = %f", i, distance_and_entity_pairs[i][1]))

                -- end

                if #distance_and_entity_pairs > 0 then
                    table.sort(distance_and_entity_pairs, compare)
                    -- Shared.Message(string.format("closest distance = %f", distance_and_entity_pairs[1][1]))
                    for i = 1, #distance_and_entity_pairs do
                        local not_space_for_self = Shared.CollideBox(self:GetExtents(), distance_and_entity_pairs[i][2]:GetOrigin() + Vector(0, 0.1, 0), CollisionRep.Move, PhysicsMask.All, EntityFilterOne(distance_and_entity_pairs[i][2]))
                        if not not_space_for_self then
                            self:SetOrigin(distance_and_entity_pairs[i][2]:GetOrigin() + Vector(0, 0.1, 0))
                            local distance_moved = (self:GetOrigin() - current_location):GetLength()
                            if distance_moved > 0.1 then
                                self.moved = true
                                return true
                            end
                        end
            ...
            until self.moved or distance_moved > 0.1 or count > 20
...
function Alien:HandleButtons(input)
    ...
    if self:isa("Lerk") then
        ...
        if self.is_carrying_an_entity == false then
            if bit.band(input.commands, Move.Weapon4) ~= 0 then
                local ray = Shared.TraceCapsule(self:GetEyePos(), self:GetEyePos() + GetNormalizedVector(self:GetViewAngles():GetCoords().zAxis), 0.25, 0.25, CollisionRep.Damage, PhysicsMask.Bullets, EntityFilterOne(self))
                if ray.entity and (ray.entity:isa("Gorge") or (ray.entity:isa("Marine") and not ray.entity:isa("JetpackMarine") and not ray.entity:isa("Exo"))) and (ray.entity.carrying_lerk_id == nil or ray.entity.carrying_lerk_id == 0) then
                    ray.entity.carrying_lerk_id = self:GetId()
                    self.is_carrying_an_entity = true
                    self.carried_entity_id = ray.entity:GetId()
                elseif not ray.entity then
                    local gorges = GetEntitiesForTeamWithinRange("Gorge", kTeam2Index, self:GetOrigin(), 3.25)
                    local marines = GetEntitiesForTeamWithinRange("Marine", kTeam2Index, self:GetOrigin(), 3.25)
                    if #gorges > 0 then
                        gorges[1].carrying_lerk_id = self:GetId()
                        self.is_carrying_an_entity = true
                        self.carried_entity_id = gorges[1]:GetId()
                    elseif #marines > 0 then
                        local non_jp_marines = {}
                        for i = 1, #marines do
                            if not marines[i]:isa("JetpackMarine") then
                                table.insert(non_jp_marines, marines[i])
                            end
                        end
                        if #non_jp_marines > 0 then
                            non_jp_marines[1].carrying_lerk_id = self:GetId()
                            self.is_carrying_an_entity = true
                            self.carried_entity_id = non_jp_marines[1]
                        end
                    end
                end
            end
        else
            if bit.band(input.commands, bit.bor(Move.Buy, Move.Weapon5)) ~= 0 then

                if self.carried_entity_id and IsValid(Shared.GetEntity(self.carried_entity_id)) then
                
                    local entity_extents = Shared.GetEntity(self.carried_entity_id):GetExtents()

                    if not Shared.CollideBox(1.5 * entity_extents, Shared.GetEntity(self.carried_entity_id):GetOrigin() - Vector(0, 0.15, 0), CollisionRep.Move, PhysicsMask.All, EntityFilterTwo(self, Shared.GetEntity(self.carried_entity_id))) then
                        self.is_carrying_an_entity = false
                        Shared.GetEntity(self.carried_entity_id).carrying_lerk_id = nil
                        self.carried_entity_id = nil
                    end

                elseif self.carried_entity_id and not IsValid(Shared.GetEntity(self.carried_entity_id)) and self.is_carrying_an_entity and Shared.GetEntity(Self.carried_entity_id).carried_entity_id ~= nil then
    ...
    if self:isa("Gorge") then
        ...
        if bit.band(input.commands, Move.Reload) ~= 0 then
            ...
            if Shared.GetTime() >= self.last_unstuck_time + 3 and self.carrying_lerk_id and self.carrying_lerk_id ~= 0 then

                self.last_unstuck_time = Shared.GetTime()

                local lerks = GetEntitiesForTeam("Lerk", kTeam2Index)
                if #lerks > 0 then
                    for i = 1, #lerks do
                        if lerks[i]:GetId() == self.carrying_lerk_id and lerks[i].carried_entity_id == self:GetId() then
                            lerks[i].is_carrying_an_entity = false
                            self.carrying_lerk_id = nil
                            lerks[i].carried_entity_id = nil
                            break
                        end
                    end
                end

                -- Shared.Message("about to call unstick_myself")

                local try_to_unstick = self:Unstick_Myself(input)
                -- if try_to_unstick then
                --     Shared.Message("Unstuck Myself")
                -- else
                --     Shared.Message("Failed To Unstick Myself")
                -- end
                
                -- Shared.Message("called unstick_myself")
        ...
        if self.carrying_lerk_id ~= 0 then
