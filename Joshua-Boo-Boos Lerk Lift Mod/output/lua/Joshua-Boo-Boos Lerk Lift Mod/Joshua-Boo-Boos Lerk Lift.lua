...
function Alien:HandleButtons(input)
    ...
    if bit.band(input.commands, Move.Reload) ~= 0 and self:isa("Gorge") then

        local gorges_nearby = GetEntitiesForTeamWithinRange("Gorge", kTeam2Index, self:GetOrigin(), 1.8)

        if not self.target_lerk and #gorges_nearby == 1 then

            local ray = Shared.TraceCapsule(self:GetEyePos(), self:GetEyePos() + 0.5 * GetNormalizedVector(self:GetViewAngles():GetCoords().zAxis), 0.2, 0.5, CollisionRep.Damage, PhysicsMask.Bullets, EntityFilterOneAndIsa(self, "Gorge"))
            if ray.fraction ~= 1 and ray.entity and ray.entity:GetIsAlive() and ray.entity:isa("Lerk") then
                self.target_lerk = ray.entity
            end

        end

        if self.target_lerk and self.target_lerk:GetIsAlive() then

            self:SetOrigin(self.target_lerk:GetOrigin() + Vector(0, 0.8, 0))

        elseif self.target_lerk and not self.target_lerk:GetIsAlive() then

            self.target_lerk = nil
