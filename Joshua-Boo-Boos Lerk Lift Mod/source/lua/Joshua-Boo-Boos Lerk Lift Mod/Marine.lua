...
function Marine:Unstick_Myself(input)

    local teammate_entities = GetEntitiesForTeam("Player", kTeam1Index)
    ...
    if #alive_teammates > 0 then
        ...
        if Shared.CollideBox(entity_extents, self:GetOrigin() + Vector(0, 0.05, 0), CollisionRep.Move, PhysicsMask.All, EntityFilterOne(self)) then
            ...
            repeat
                ...
                local unsure_teammates = GetEntitiesForTeam("Player", kTeam1Index)
...
function Marine:HandleButtons(input)
    ...
    if bit.band(input.commands, Move.Taunt) ~= 0 then
        ...
        if Shared.GetTime() >= self.last_unstuck_time + 3 and self.carrying_lerk_id == nil then
