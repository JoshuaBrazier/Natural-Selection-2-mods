...
function GUIAlienHUD:Initialize()
    ...
    self.lerk_lift_text = self:CreateAnimatedTextItem()
    self.lerk_lift_text:SetFontName(GUIMarineHUD.kTextFontName)
    self.lerk_lift_text:SetFontIsBold(true)
    self.lerk_lift_text:SetLayer(kGUILayerPlayerHUDForeground2)
    self.lerk_lift_text:SetColor(kAlienTeamColorFloat)
    self.lerk_lift_text:SetIsScaling(false)

    self.lerk_lift_text_name = self:CreateAnimatedTextItem()
    self.lerk_lift_text_name:SetFontName(GUIMarineHUD.kTextFontName)
    self.lerk_lift_text_name:SetFontIsBold(true)
    self.lerk_lift_text_name:SetLayer(kGUILayerPlayerHUDForeground2)
    self.lerk_lift_text_name:SetColor(kAlienTeamColorFloat)
    self.lerk_lift_text_name:SetIsScaling(false)
...
function GUIAlienHUD:Reset()
    ...
    self.lerk_lift_text:SetFontName(GUIMarineHUD.kTextFontName)
    self.lerk_lift_text:SetScale(GetScaledVector())
    self.lerk_lift_text:SetPosition(Vector(Client.GetScreenWidth() - ((1920 - 700) * self.scale), Client.GetScreenHeight() - (265 * self.scale), 0) * (1/self.scale))
    GUIMakeFontScale(self.lerk_lift_text)

    self.lerk_lift_text_name:SetFontName(GUIMarineHUD.kTextFontName)
    self.lerk_lift_text_name:SetScale(GetScaledVector())
    self.lerk_lift_text_name:SetPosition(Vector(Client.GetScreenWidth() - ((1920 - 800) * self.scale), Client.GetScreenHeight() - (205 * self.scale), 0) * (1/self.scale))
    GUIMakeFontScale(self.lerk_lift_text_name)
...
function GUIAlienHUD:Update(deltaTime)
    ...
    if player then
        ...
        if player:isa("Lerk") then
            ...
            self.lerk_lift_text:SetText("Press 4 to grab a gorge / non-jp marine - Press 5 to let go.")
            self.lerk_lift_text:SetIsVisible(true)

            if player.carried_entity_id and player.carried_entity_id ~= 0 and IsValid(Shared.GetEntity(player.carried_entity_id)) then

                if Shared.GetEntity(player.carried_entity_id):isa("Gorge") then
                    self.lerk_lift_text_name:SetText(string.format("Carrying GORGE named %s", Shared.GetEntity(player.carried_entity_id):GetName()))
                elseif Shared.GetEntity(player.carried_entity_id):isa("Marine") then
                    self.lerk_lift_text_name:SetText(string.format("Carrying MARINE named %s", Shared.GetEntity(player.carried_entity_id):GetName()))
                end

                self.lerk_lift_text_name:SetIsVisible(true)

            else

                self.lerk_lift_text_name:SetText("")
                self.lerk_lift_text_name:SetIsVisible(false)

            end

        else

            self.lerk_lift_text:SetIsVisible(false)
            self.lerk_lift_text_name:SetIsVisible(false)
