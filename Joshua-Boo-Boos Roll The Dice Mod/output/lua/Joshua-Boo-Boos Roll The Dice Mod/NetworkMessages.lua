        ...
        function(client, msg)
            ...
            if gameRules then
                if gameRules:GetGameState() == kGameState.Started then
                    if Shared.GetTime() - gameRules:GetGameStartTime() >= 14.5 then
                                ...
                                playerEntity:AddTimedCallback(function(playerEntity)
                                                                ...
                                                                if not playerEntity:isa("MarineSpectator") and not playerEntity:isa("AlienSpectator") then
                                                                    ...
                                                                    elseif rolled_value >= 2 and rolled_value < 15 then
                                                                        ...
                                                                        if Shared.GetTime() - gameRules:GetGameStartTime() >= 300 then
                                                                                ...
                                                                                if not playerEntity:isa("Exo") then
                                                                                    ...
                                                                                    for i = 1, #weapons do
                                                                                        if weapons[i]:isa("ClipWeapon") then
                                                                                            weapons[i].clip = 4 * weapons[i]:GetClipSize()
                                                                                            weapons[i].ammo = 0
                                                                                        end
                                                                                    end
                                                                                    for i = 1, #players do
                                                                                        if IsValid(players[i]) then
                                                                                            players[i]:SendDirectMessage(string.format("Player %s rolled a %i and their clip-weapons have four clips of ammo in one and no spare ammo!", playerEntity:GetName(), rolled_value))
                                                                                        end
                                                                                    end
                                                                                else
                        ...
                        if IsValid(Shared.GetEntity(tonumber(msg.entId))) then
                            ...
                            if not playerEntity.lastCalledRTDTime or playerEntity.lastCalledRTDTime and playerEntity.lastCalledRTDTime ~= nil and msg.rollTime >= playerEntity.lastCalledRTDTime + kTimePerRoll then
                                ...
                                if IsValid(Shared.GetEntity(tonumber(msg.entId))) then
                                    local playerEntity = Shared.GetEntity(tonumber(msg.entId))
                                    playerEntity:SendDirectMessage("Please wait until 15 seconds after the start of the round before rolling the dice.")
                                end
                    ...
                    if IsValid(Shared.GetEntity(tonumber(msg.entId))) then
                        ...
                        if not playerEntity.lastCalledRTDTime or playerEntity.lastCalledRTDTime and playerEntity.lastCalledRTDTime ~= nil and msg.rollTime >= playerEntity.lastCalledRTDTime + kTimePerRoll then
                            ...
                            if IsValid(Shared.GetEntity(tonumber(msg.entId))) then
                                local playerEntity = Shared.GetEntity(tonumber(msg.entId))
                                playerEntity:SendDirectMessage("Please only try to roll the dice when the round is ongoing.")
                            end
