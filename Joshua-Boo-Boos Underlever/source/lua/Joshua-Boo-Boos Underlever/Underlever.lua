    ...
    if self.clip > 0 then
        ...
        if tagName == "shoot" then
            ...
            if Server then
                ...
                CreateEntity(Underlever.kMapName, self:GetParent():GetEyePos() + self:GetParent():GetViewAngles():GetCoords().xAxis * 0.3 + self:GetParent():GetViewAngles():GetCoords().yAxis * (-0.4) + self:GetParent():GetViewAngles():GetCoords().zAxis * 0.1)
                DestroyEntity(self)
