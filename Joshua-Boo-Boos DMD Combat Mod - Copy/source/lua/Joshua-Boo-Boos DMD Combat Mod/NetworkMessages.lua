...
if Server then
    ...
    function(client, msg)
        ...
        if not playerEntity.lastCalledRTD then
            playerEntity.lastCalledRTD = Shared.GetTime()
            ...
            if Shared.GetTime() >= playerEntity.lastCalledRTD + 5 then

                playerEntity.lastCalledRTD = Shared.GetTime()

                if playerEntity.RTDSpamCount then

                    playerEntity.RTDSpamCount = math.max(0,playerEntity.RTDSpamCount - 1)
                ...
                if playerEntity.RTDSpamCount then
                    if playerEntity.RTDSpamCount == 3 then
                        playerEntity:Kill()
                        for i = 1, #players do
                            players[i]:SendDirectMessage(string.format("Player %s was killed for continuously spamming !rtd!", playerEntity:GetName()))
                        end
                    else
                        playerEntity.RTDSpamCount = playerEntity.RTDSpamCount + 1
                        for i = 1, #players do
                            players[i]:SendDirectMessage(string.format("Player %s is spamming roll the dice (%i of 3)!", playerEntity:GetName(), playerEntity.RTDSpamCount))
                        end
                    end
                else
                    playerEntity.RTDSpamCount = 1
                    for i = 1, #players do
                        players[i]:SendDirectMessage(string.format("Player %s is spamming roll the dice (%i of 3)!", playerEntity:GetName(), playerEntity.RTDSpamCount))
                    end
                end
