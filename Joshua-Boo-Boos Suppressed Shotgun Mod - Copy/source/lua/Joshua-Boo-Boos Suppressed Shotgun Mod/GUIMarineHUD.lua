...
function GUIMarineHUD:Initialize()
    ...
    self.sg_text = self:CreateAnimatedTextItem()
    self.sg_text:SetFontName(GUIMarineHUD.kTextFontName)
    self.sg_text:SetTextAlignmentX(GUIItem.Align_Min)
    self.sg_text:SetTextAlignmentY(GUIItem.Align_Min)
    self.sg_text:SetLayer(kGUILayerPlayerHUDForeground2)
...
function GUIMarineHUD:Update(deltaTime)
    ...
    if weapon and weapon:isa("SuppressedShotgun") then
        ...
        elseif weapon.shotgun_cartridge == "buckshot" then
            result = "BUCKSHOT"
            color = Color(1,165/255,0,1)
        elseif weapon.shotgun_cartridge == "incendiary" then
            result = "INCENDIARY"
            color = Color(1,0,0,1)
        ...
        self.sg_text:SetColor(color)
        self.sg_text:SetIsVisible(true)

        -- Shared.Message("weapon.tracked_target:" .. string.format("%s", weapon.tracked_target))

        if weapon.tracked_target ~= 0 and Shared.GetTime() < weapon.hit_tracker_target_at + 5 then
    ...
    elseif weapon and not weapon:isa("SuppressedShotgun") then
