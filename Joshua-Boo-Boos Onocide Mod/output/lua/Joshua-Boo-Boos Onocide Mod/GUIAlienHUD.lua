...
function GUIAlienHUD:Initialize()
    ...
    self.onocide_text = self:CreateAnimatedTextItem()
    self.onocide_text:SetFontName(GUIMarineHUD.kTextFontName)
    self.onocide_text:SetFontIsBold(true)
    self.onocide_text:SetLayer(kGUILayerPlayerHUDForeground2)
    self.onocide_text:SetColor(kAlienTeamColorFloat)
    self.onocide_text:SetIsScaling(false)
...
function GUIAlienHUD:Reset()
    ...
    self.onocide_text:SetFontName(GUIMarineHUD.kTextFontName)
    self.onocide_text:SetScale(GetScaledVector())
    self.onocide_text:SetPosition(Vector(Client.GetScreenWidth() - ((1920 - 800) * self.scale), Client.GetScreenHeight() - (315 * self.scale), 0) * (1/self.scale))
    GUIMakeFontScale(self.onocide_text)
...
function GUIAlienHUD:Update(deltaTime)
    ...
    if player then
        ...
        if player:isa("Onos") then
            ...
            if kCombatVersion and player.threeHives then

                self.onocide_text:SetText("Press your reload key to go boom.")

            elseif not kCombatVersion then
            ...
            self.onocide_text:SetIsVisible(true)

        else

            self.onocide_text:SetIsVisible(false)
