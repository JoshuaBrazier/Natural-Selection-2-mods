...
local kLerkSound = PrecacheAsset("sound/primal_scream.fev/primal_scream/primal_scream")
...
function Lerk:HandleButtons(input)
    ...
    local lerk_energy = self:GetEnergy()
    local max_energy = self:GetMaxEnergy()

    if bit.band(input.commands, Move.Reload) ~= 0 and lerk_energy >= 0.5*max_energy then

        self.time_now = Shared.GetTime()

        local has_shift_hive = false

        local techtree = GetTechTree(kTeam2Index)

        if techtree then
            if techtree:GetHasTech(kTechId.ShiftHive) then
                has_shift_hive = true
            end
        end
        
        -- local pres_amount = self:GetResources()

        -- if has_shift_hive and pres_amount >= 2 and self.time_now >= self.last_primal_scream then

        if has_shift_hive and self.time_now >= self.last_primal_scream then

            self.last_primal_scream = Shared.GetTime() + 2.3

            -- if Server then
            --     self:SetResources(pres_amount - 2)
            -- end

            self:SetEnergy(math.max(0, lerk_energy - (0.5*max_energy)))

            if Server then

                self:TriggerEnzyme(2.3)
                StartSoundEffectOnEntity(kLerkSound, self, 0.15)

            end

            -- local nearby_aliens = GetEntitiesForTeamWithinRange("Player", self:GetTeamNumber(), self:GetOrigin(), 10)

            -- if #nearby_aliens > 0 then
            --     if Server then
            --         for i, alien in ipairs(nearby_aliens) do
            --             alien:TriggerEnzyme(2.3)
            --         end
            --     end
            -- end
