...
local sentry_damage_scalar = 1 -- 3.5
...
function FireMixin:UpdateFireState()
    ...
    if Server and self:GetIsOnFire() then
        ...
        if self:GetIsAlive() and (not self.timeLastFireDamageUpdate or self.timeLastFireDamageUpdate + kBurnUpdateRate <= time) then
            ...
            if doer and doer:isa("Sentry") then
                damageOverTime = sentry_damage_scalar * kBurnUpdateRate * kBurnDamagePerSecond
            else
                damageOverTime = kBurnUpdateRate * kBurnDamagePerSecond
            end
