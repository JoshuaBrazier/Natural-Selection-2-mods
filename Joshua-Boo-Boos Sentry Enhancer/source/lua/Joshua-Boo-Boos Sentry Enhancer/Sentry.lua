...
local kAttackSoundName = PrecacheAsset("sound/NS2.fev/marine/heavy/railgun_fire")
local kMinigunExoSound = PrecacheAsset("sound/NS2.fev/marine/heavy/spin_up")
...
Sentry.kDamage = 10 --kSentryDamage
Sentry.kRange = kSentryRange
Sentry.kBaseROF = 2 * kSentryAttackBaseROF
Sentry.kRandROF = kSentryAttackRandROF
Sentry.kSpread = 0.25 * kSentrySpread
...
function Sentry:GetTracerEffectName()
    return kRailgunTracerEffectName
end

function Sentry:GetTracerResidueEffectName()
...
if Server then
    ...
    function Sentry:FireBullets()
        ...
        StartSoundEffectOnEntity(kAttackSoundName, self, 0.45)
        ...
        for bullet = 1, self.kBulletsPerSalvo do
            ...
            if trace.fraction < 1 then
                ...
                if trace.entity then
                    if trace.entity:isa("Alien") and trace.entity:isa("Player") then
                        local energy = trace.entity:GetEnergy()
                        local max_energy = trace.entity:GetMaxEnergy()
                        trace.entity:SetEnergy(math.max(0, energy - 0.03 * max_energy))
                    end
        ...
        if self.timeNextAttack == nil or (Shared.GetTime() > self.timeNextAttack) then
            ...
            if self.target then
                ...
                if not initialAttack then
                    ...
                    self.attacking = true
                    StartSoundEffectOnEntity(kMinigunExoSound, self, 1)
                    self:FireBullets()
            ...
            elseif self.attacking then
                ...
                StartSoundEffectOnEntity(kAttackSoundName, self, 0.4)
