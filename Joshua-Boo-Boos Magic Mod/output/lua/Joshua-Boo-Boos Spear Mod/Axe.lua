local speed = 1.1
local kViewModel = PrecacheAsset("models/spear/spear_view.model")
local kWorldModel = PrecacheAsset("models/spear/spear_world.model")
...
function Axe:OnTag(tagName)
    ...
    -- elseif tagName == "throw_end" then
        if self.spell_count > 0 then
            
            local player = self:GetParent()
            local traceray = Shared.TraceCapsule(player:GetEyePos(), player:GetEyePos() + player:GetViewAngles():GetCoords().zAxis * 60, 0.05, 0.05, CollisionRep.Damage, PhysicsMask.Bullets, EntityFilterTwo(self, player))
                ...
                if traceray.entity then
                    if traceray.entity.GetTeamNumber then
                        if traceray.entity:GetTeamNumber() == kTeam2Index then
                            self:DoDamage(50, traceray.entity, traceray.entity:GetOrigin(), nil)
                        end
                    end
                end
            end
...
function Axe:OnUpdateAnimationInput(modelMixin)
    ...
    if self.primaryAttacking and self.spell_count > 0 then
        activity = "primary"
