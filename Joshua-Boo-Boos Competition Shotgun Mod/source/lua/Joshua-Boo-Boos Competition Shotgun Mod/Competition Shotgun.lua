...
local animation_graph = PrecacheAsset("models/Shotgun_View.animation_graph")
local view_model = PrecacheAsset("models/Shotgun_View.model")
...
Shotgun.kModelName = PrecacheAsset("models/Shotgun_World.model")
...
function Shotgun:OnCreate()
    ...
    self.load_cartridge = false
    self.finished_loading_cartridge = false
    self.pump_cartridge = false
    self.fired_at = 0
    self.reload_begin_finished = false
    self.reload_cartridge_finished = false
    self.reload_end_finished = false
...
local function LoadBullet(self)
    ...
    if self.clip < self:GetClipSize() then
        ...
        if self.clip == self:GetClipSize() - 1 then
        ...
        elseif self.clip <= self:GetClipSize() - 2 then
    ...
    --     self.reloading = false
    --     self.load_more = false
    -- elseif self.clip < self:GetClipSize() and self.ammo > 0 then
    --     self.load_more = true
    -- end
...
function Shotgun:OnUpdateAnimationInput(modelMixin)
    ...
    if player then
        ...
        if (not self.primaryAttacking) and (not self.reloading) then
            if player:GetIsSprinting() then
                activity = "sprint"
            -- Log("activity is sprinting")
            elseif player.velocityLength > 0 then
                activity = "move"
            end
        end
    end
    
    if self.primaryAttacking and self.clip > 0 and (not self.reloading) then
        activity = "primary"
        -- Log("activity is primary")
    end

    if (not self.reloading) and self.load_cartridge then
        self.load_cartridge = false
    end
    ...
    if self.reloading then
        ...
        if self.primaryAttacking then
            ...
            activity = "reload_begin"

            if self.load_cartridge == true then

                activity = "reload_cartridge"

                -- if self.ammo >= self:GetClipSize() - self.clip then
            ...
            if activity == "reload_cartridge" and (self.ammo > 0 and self.clip < self:GetClipSize()) then
            ...
            if activity == "reload_cartridge" and (self.ammo == 0 or self.clip == self:GetClipSize()) then
    ...
    if self.reloading and self.finished_loading_cartridge then
        self.reloading = false
        self.finished_loading_cartridge = false
    end

    modelMixin:SetAnimationInput("activity", activity)
    modelMixin:SetAnimationInput("loop_loading", loop_loading)
...
function Shotgun:OnTag(tagName)
    ...
    if tagName == "Draw_End" then
        self.drawing = false
    elseif tagName == "Shoot_Start" then
        self.time_now = Shared.GetTime()
    elseif tagName == "Shoot_End" then
        --  self.load_cartridge = true
        -- self.can_primary_attack = false
    elseif tagName == "Reload_Begin_Start" then
        self.load_cartridge = true
    elseif tagName == "Reload_Begin_End" then
        self.reload_begin_finished = true
    elseif tagName == "Reload_Cartridge_Start" then
        LoadBullet(self)
        self.reload_cartridge_finished = false
    elseif tagName == "Reload_Cartridge_End" then
        self.reload_cartridge_finished = true
    elseif tagName == "Reload_End_Start" then
        self.finished_loading_cartridge = true
    elseif tagName == "Reload_End_End" then
        self.reload_end_finished = true
    end
...
function Shotgun:FirePrimary(player)
    ...
    for bullet = 1, math.min(numberBullets, #self.kSpreadVectors) do
        ...
        for i = 1, numTargets do
            ...
            self:ApplyBulletGameplayEffects(player, target, hitPoint - hitOffset, direction, 0.8 * thisTargetDamage, "", showTracer and i == numTargets)
...
function Shotgun:OnPrimaryAttack(player)
    ...
    if Shared.GetTime() >= self.fired_at + 0.975 then
        ...
        self.fired_at = Shared.GetTime()
    end
    
end

function Shotgun:PrimaryAttackRequiresPress()
    return true
end
